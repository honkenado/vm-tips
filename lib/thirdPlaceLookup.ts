...
export function getThirdPlaceMapping(letters: string[]): string[] {
  const key = letters.slice().sort().join("");

  const lines = RAW_THIRD_PLACE_CSV
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter(Boolean);

  for (const line of lines) {
    const [csvKey, ...values] = line.split(";").map((p) => p.trim());
  ...
  console.error("Ingen mapping hittad för:", key);
  return [];
}